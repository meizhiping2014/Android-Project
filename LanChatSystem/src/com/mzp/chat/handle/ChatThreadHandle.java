...
import com.mzp.chat.activity.BaseActivity;
 ...
 *	2014-7-24
 */
public class ChatThreadHandle implements Runnable {
	public static final String TAG = "ChatThreadHandle";
	private static ChatThreadHandle instance;
	private static final int BUFFERLENGTH = 1024; //缓冲大小
	private DatagramSocket udpSocket;//用于接收和发送UDP数据的socket
	private DatagramPacket sendData;//用于发送的udp数据包
	private DatagramPacket receiveData;//用于接收的udp数据包
	private Map<String,UserInfo> users;	//当前所有用户的集合，以IP为KEY
	...
	private byte[] sendBuffer = null;//发送的数据字节
	private ChatApplication chatApp;//全局对象
	private ExecutorService executor=Executors.newFixedThreadPool(20);//线程池
	/**
	 * 单例模式
	 * @return
	 */
	public static ChatThreadHandle newInstance(){
		if(instance == null)
			instance = new ChatThreadHandle();
		return instance;
	}
	...
	public void run() {
		...
		while(onWork){
			...
			String ipmsgStr = "";
			try {
				ipmsgStr = new String(resBuffer, 0, receiveData.getLength(),"gbk");
			...
			case IpMessageConst.IPMSG_BR_ENTRY:	{	//收到上线数据包，添加用户，并回送IPMSG_ANSENTRY应答。
				...
				String currentIp = receiveData.getAddress().toString();
				Log.e(TAG, "==============接收的ip："+currentIp+"端口号："+receiveData.getPort());
				BaseActivity.sendEmptyMessage(IpMessageConst.IPMSG_BR_ENTRY);
				if(!chatApp.getLocalIp().equals(currentIp)){
					//下面构造回送报文内容
					...
					sendUdpData(ipmsgSend.getProtocolString(), receiveData.getAddress(), receiveData.getPort());	//发送数据
				}
			}	
				break;
			...
			case IpMessageConst.IPMSG_BR_EXIT:{	//收到下线广播，删除users中对应的值ֵ
				String userIp = receiveData.getAddress().getHostAddress();
				users.remove(userIp);
			...
			case IpMessageConst.IPMSG_SENDMSG:{ //收到消息，处理
				String senderIp = receiveData.getAddress().getHostAddress();	//得到发送者IP
				...
				// 若只是发送消息，处理消息
				boolean selfMsg = false;
				if(senderIp.equals(chatApp.getLocalIp())){
					selfMsg = true;
				}
				ChatMessage msg = new ChatMessage(senderIp, senderName, msgStr, time,selfMsg);
			...
			if(receiveData != null){	//每次接收完UDP数据后，重置长度。否则可能会导致下次收到数据包被截断。
				receiveData.setLength(BUFFERLENGTH);
			}
		}
	...
	public boolean connectSocket(){	//监听端口，接收UDP数据
		...
		try {
			...
			if(receiveData == null)
				receiveData = new DatagramPacket(resBuffer, BUFFERLENGTH);
	 ...
	 * @param sendPort		端口
	 */
	public synchronized void sendUdpData(final String sendStr, final InetAddress sendto, final int sendPort){	
		executor.execute(new Runnable() {
			...
			public void run() {
				try {
					...
					// 构造发送的UDP数据包
					sendData = new DatagramPacket(sendBuffer, sendBuffer.length, sendto, sendPort);
					udpSocket.send(sendData);	//发送udp数据包
...
//		user.setUserName(ipmsgPro.getSenderName());
		...
		if(userInfo.length < 1){
			user.setUserName(ipmsgPro.getSenderName());
			if(userIp.equals(chatApp.getLocalIp()))
